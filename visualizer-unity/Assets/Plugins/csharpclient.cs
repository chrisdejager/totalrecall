...
public class csharpclient {
  private static bool endExpt=false;
	BufferClientClock C;
  public csharpclient(String[] args) {
	 ...
	 if (args.Length>=2) {
		...
		if (port <= 0) {			
			Debug.Log("Second parameter ("+args[1]+") is not a valid port number.");
		  return;
		}
	 }
	 if (args.Length>=3) {
	 ...
	 while( hdr==null ) {
		...
		if ( hdr==null ) {
				Debug.Log("Invalid Header... waiting");
	 ...
	 while ( !endExpt ) {
		...
		if ( sec.nEvents > nEvents ){
		  ...
		  // filter for ones we want
				Debug.Log("Got " + evs.Length + " events");
		  for ( int ei=0; ei<evs.Length; ei++){
			 ...
			 if ( !evttype.Equals("echo") ){  // N.B. use equals, not == to compare string contents!
				...
				// Print the event to the console
						Debug.Log(ei + ") t:" + evt.getType().toString() + " v:" + evt.getValue().toString() + " s:" + evt.sample);
				...
				Debug.Log("Timeout waiting for events");
		}
	 }
